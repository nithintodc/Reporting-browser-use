...
Reads the first "Day-Slot - {storeID}" sheet to get:
- store_id from the sheet name
- Day, Slot, Min.Subtotal from the first data row (columns Day, Slot, Min.Subtotal).
"""
...
def get_campaign_params_from_combined_analysis(combined_xlsx_path: Path) -> Optional[dict]:
    ...
    # Campaign name: e.g. 14351-Lunch-Wednesday
    campaign_name = f"{store_id}-{slot}-{day}"
